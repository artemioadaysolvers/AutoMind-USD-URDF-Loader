...
function isMovable(j) {
    ...
    if (j.exportedMovable === true && !/FixedJoint/i.test(j.schema || ''))
        return true;
    if (/revolute|continuous|hinge|prismatic/i.test(t))
        return true;
...
function getJointValue(j) {
    const raw = (Array.isArray(j?.jointValue) ? j.jointValue[0] : undefined) ?? j?.value ?? (isPrismatic(j) ? j?.position : j?.angle) ?? 0;
    return Number(raw) || 0;
}
function setJointValue(robot, j, v) {
    ...
    if (typeof j.setJointValue === 'function')
        ...
        robot.setJointValue(j.name, v);
    robot.updateMatrixWorld?.(true);
}
function collectMeshesInLink(linkObj) {
    const out = [];
    linkObj?.traverse?.(o => { if (o?.isMesh && o.geometry && !o.userData.__isHoverOverlay)
        out.push(o); });
    return out;
}
...
function meshPickableNow(mesh, plane) {
    if (!mesh || !mesh.isMesh || !mesh.geometry)
        return false;
    if (mesh.visible === false || mesh.userData?.__isHoverOverlay)
        return false;
    if (mesh.userData?.__automindVisibilityTarget === false)
        return false;
    if (meshOpacityForPicking(mesh) <= 0.035)
        return false;
...
function meshHasKeptGeometryBySection(mesh, plane, eps = 1e-7) {
    ...
    if (!pos || !pos.count)
        return meshVisibleBySectionBBox(mesh, plane, eps);
    ...
    for (let i = 0; i < count; i += step) {
        ...
        if (sectionKeepsPoint(__sectionVertexTmp, plane, eps))
            return true;
    }
...
function findAncestorLink(o, linkSet) { while (o) {
    if (linkSet.has(o))
        return o;
    o = o.parent;
} return null; }
function buildHoverOverlay({ color = HOVER_COLOR, opacity = HOVER_OPACITY, getSectionPlane = null } = {}) {
    ...
    function showLink(link) {
        ...
        for (const m of collectMeshesInLink(link)) {
            if (meshPickableNow(m, plane))
                showMesh(m);
        }
    }
    return { clear, showMesh, showLink };
}
export function attachInteraction({ scene, camera, renderer, controls, robot, selectMode = 'link', getSectionPlane = null, onSelectLink = null }) {
    if (!scene || !camera || !renderer || !controls)
        throw new Error('[USD SelectionAndDrag] Missing core objects');
    ...
    function setSelected(link, mesh = null) {
        ...
        if (selectMode === 'mesh' && mesh)
            selectedMeshes = [mesh];
        else
            selectedMeshes = link ? collectMeshesInLink(link) : [];
        refreshSelectionMarker();
    }
    function clearSelection() { selectedMeshes = []; selectedLink = null; if (selectionHelper)
        selectionHelper.visible = false; }
    ...
    function linkNameFromObject(obj) {
        let o = obj;
        while (o) {
            if (o.userData?.__linkName)
                return o.userData.__linkName;
            o = o.parent;
        }
        return '';
    }
    function pickInfoFromPointer(ev) {
        ...
        robotModel?.traverse?.(o => {
            if (meshPickableNow(o, plane))
                pickables.push(o);
        });
        const hits = raycaster.intersectObjects(pickables, true);
        for (const hit of hits) {
            if (!pointVisibleBySection(hit.point, plane))
                continue;
    ...
    function getManipulableJointForLink(link) {
        ...
        if (typeof robotModel.getManipulableJointForLinkName === 'function')
            return robotModel.getManipulableJointForLinkName(name);
        let n = link;
        while (n) {
    ...
    function getJointWorldPivot(j) {
        if (robotModel?.getJointWorldPivot)
            return robotModel.getJointWorldPivot(j);
        return j?.getWorldPosition ? j.getWorldPosition(new THREE.Vector3()) : new THREE.Vector3();
    }
    function getJointWorldAxis(j) {
        if (robotModel?.getJointWorldAxis)
            return robotModel.getJointWorldAxis(j);
    ...
    function getRevoluteDragDelta(j, startPoint, endPoint, initialGrabPoint) {
        ...
        if (Math.abs(dragTmp.dot(dragPlane.normal)) > 0.3) {
            ...
            dragProjectedStart.sub(dragPivotWorld);
            dragProjectedEnd.sub(dragPivotWorld);
            if (dragProjectedStart.lengthSq() < 1e-12 || dragProjectedEnd.lengthSq() < 1e-12)
                return 0;
    ...
    function cancelActiveDrag(ev) {
        ...
        try {
            robotModel?.endInteractiveDrag?.(d.joint);
        }
        catch (_) { }
        activeDrag = null;
    ...
    function updateJointDrag(ev) {
        ...
        if (Number.isFinite(delta) && Math.abs(delta) > 1e-9)
            setJointValue(robotModel, d.joint, getJointValue(d.joint) + delta);
        d.prevRay.copy(ray);
        refreshSelectionMarker();
        return true;
    }
        ...
        if (pending && !pending.moved) {
            try {
                if (typeof onSelectLink === 'function')
                    onSelectLink(pending.link, pending.pick);
            }
            catch (_) { }
        }
    }
    function isolateSelected() {
        if (!robotModel || !selectedLink)
            return;
        if (isolated) {
            robotModel.traverse(o => { if (o.isMesh && o.geometry)
                o.visible = true; });
            isolated = false;
            refreshSelectionMarker();
            return;
    ...
    function onMove(ev) {
        ...
        const planeNow = getPlane();
        if (lastHover && !linkPickableNow(lastHover, planeNow)) {
            hover.clear();
            lastHover = null;
            renderer.domElement.style.cursor = 'auto';
        }
        const now = performance.now();
        if (now - lastHoverRaycastAt < 34)
            return;
    ...
    function onDown(ev) {
        ...
        if (startJointDrag(ev, pick)) {
            ...
            try {
                if (typeof onSelectLink === 'function')
                    onSelectLink(pick.link, pick);
            }
            catch (_) { }
        }
    }
    function onUp(ev) { if (activeDrag) {
        ev.preventDefault?.();
        endJointDrag(ev);
    } }
    function onKey(ev) { if (String(ev.key || '').toLowerCase() === 'i')
        isolateSelected(); }
