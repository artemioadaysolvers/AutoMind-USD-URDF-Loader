...
function isolateMeshesSmooth(core, meshesToKeep, duration = 560) {
  ...
  // BUILD153: component-list selection fades Show Joints / Show Loops out with
  // the same TWEEN-style opacity suppression used by the USD viewer.
  try { core.setMechanismSuppressed?.(kept.length > 0, duration); } catch (_) { setMechanismSuppressedLocal(core, kept.length > 0, duration); }
...
let mechanismSuppressRAF = 0;
let mechanismSuppressToken = 0;
function setMechanismSuppressedLocal(core, suppressed = false, duration = 450) {
  if (!core) return;
  const start = Number.isFinite(core.__mechanismOpacity) ? core.__mechanismOpacity : 1;
  const target = suppressed ? 0 : 1;
  const token = ++mechanismSuppressToken;
  if (mechanismSuppressRAF) { try { cancelAnimationFrame(mechanismSuppressRAF); } catch (_) {} mechanismSuppressRAF = 0; }
  if (!duration || duration <= 0) {
    core.__mechanismOpacity = target;
    try { core.__mechanismDecorations?.update?.(); } catch (_) {}
    try { core.renderer?.__automindUpdateMechanismDecorations?.(); } catch (_) {}
    return;
  }
  const t0 = performance.now();
  function step(now) {
    if (token !== mechanismSuppressToken) return;
    const u = Math.min(1, (now - t0) / Math.max(1, duration));
    const k = easeInOutCubic(u);
    core.__mechanismOpacity = start + (target - start) * k;
    try { core.__mechanismDecorations?.update?.(); } catch (_) {}
    try { core.renderer?.__automindUpdateMechanismDecorations?.(); } catch (_) {}
    try { core.renderer?.render?.(core.scene, core.camera); } catch (_) {}
    if (u < 1) mechanismSuppressRAF = requestAnimationFrame(step);
    else mechanismSuppressRAF = 0;
  }
  mechanismSuppressRAF = requestAnimationFrame(step);
}
...
export function render(opts = {}) {
  ...
  app = {
    ...
    sectionPlane: null,
    getSectionPlane: () => app?.sectionPlane || null,
    setMechanismClippingPlane: (_plane) => { try { core.__mechanismClipPlane = _plane || null; app.__mechanismDecorations?.update?.(); } catch (_) {} },
    setMechanismToggles: (_toggles = {}) => { try { app.__mechanismDecorations?.set?.(_toggles); } catch (_) {} },
    setMechanismSuppressed: (suppressed = false, duration = 450) => setMechanismSuppressedLocal(core, suppressed, duration),
  ...
  try { app.__mechanismDecorations = createMechanismDecorationController(core, robot, urdfContent); core.__mechanismDecorations = app.__mechanismDecorations; } catch (e) { debugLog('mechanism decorations unavailable', String(e)); }
  try {
...
function showAll(core) {
  ...
  try { core.setMechanismSuppressed?.(false, 520); } catch (_) { setMechanismSuppressedLocal(core, false, 520); }
