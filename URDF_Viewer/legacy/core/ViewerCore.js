...
// BUILD151: finite model-sized grid and orthographic-safe depth, ported from the
// advanced USD/URDF tools pipeline.  The grid size is derived from the robot
// bounding box and never grows just because the camera zooms out.
...
function disposeGrid(grid) {
  ...
  try { (Array.isArray(grid?.material) ? grid.material : [grid?.material]).forEach(m => m?.dispose?.()); } catch (_) {}
}
...
function resizeSceneHelpersForObject(helpers, object) {
  ...
  try { helpers.axes.scale.setScalar(Math.max(modelDim * 0.35, cell * 8)); helpers.axes.position.copy(center); helpers.axes.frustumCulled = false; } catch (_) {}
}
function configureSceneShadowsForObject(root, helpers, keyLight) {
  ...
  try {
    ...
    (Array.isArray(helpers.ground.material) ? helpers.ground.material : [helpers.ground.material]).forEach(m => { if (m) m.needsUpdate = true; });
  } catch (_) {}
  try {
    keyLight.target.position.copy(center);
    keyLight.position.copy(center.clone().add(new THREE.Vector3(dim * 2.1, dim * 3.0, dim * 2.4)));
    if (!keyLight.target.parent) keyLight.parent?.add?.(keyLight.target);
  ...
  try { root.traverse(o => { if (o?.isMesh && o.geometry && !o.userData?.__isHoverOverlay) { o.castShadow = true; o.receiveShadow = true; } }); } catch (_) {}
}
...
export function createViewer({ container, background = 0xffffff, pixelRatio } = {}) {
  ...
  function onResize() {
    ...
    camera.updateProjectionMatrix();
    try { keepOrthographicDepthSafe(camera, controls, helpers, robotModel); } catch (_) {}
  ...
  function loadURDF(urdfText, { loadMeshCb } = {}) {
    ...
    if (robot && robot.isObject3D) {
      ...
      // First fit + BUILD151 finite grid/shadow context from model bounds
      setTimeout(() => {
        if (!robotModel) return;
        try { resizeSceneHelpersForObject(helpers, robotModel); configureSceneShadowsForObject(robotModel, helpers, dir); } catch (_) {}
        ...
        try { keepGridInfiniteForView(helpers, camera, controls, robotModel); } catch (_) {}
      }, 50);
    }
    return robotModel;
  }
  ...
  function setSceneToggles({ grid, ground, axes, shadows } = {}) {
    ...
    // Resize helpers to object
    if (robotModel) {
      try { resizeSceneHelpersForObject(helpers, robotModel); } catch (_) {}
      try { if (typeof shadows === 'boolean') configureSceneShadowsForObject(robotModel, helpers, dir); } catch (_) {}
      try { keepGridInfiniteForView(helpers, camera, controls, robotModel); } catch (_) {}
  ...
  function animate() {
    ...
    controls.update();
    try { keepGridInfiniteForView(helpers, camera, controls, robotModel); } catch (_) {}
    try { if (typeof renderer.__automindUpdateMechanismDecorations === 'function') renderer.__automindUpdateMechanismDecorations(); } catch (_) {}
  ...
  return {
    ...
    setProjection,
    setSceneToggles,
    refreshRobotContext: (obj = robotModel) => { try { resizeSceneHelpersForObject(helpers, obj); configureSceneShadowsForObject(obj, helpers, dir); keepGridInfiniteForView(helpers, camera, controls, obj); return true; } catch (_) { return false; } },
    setBackground,
    setPixelRatio,
    onResize,
    destroy
