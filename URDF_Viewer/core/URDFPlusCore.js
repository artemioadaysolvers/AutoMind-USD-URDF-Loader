...
// AutoMind URDF+ loader for the modular USD-style viewer shell. BUILD141: exact BUILD138 HTML visual-tree and DAE loading policy synced; no forced backup, no double DAE unit scale, robust texture preservation.
...
function textureHasResolvableSource(tex) {
  const img = tex?.image;
  const src = String(img?.currentSrc || img?.src || tex?.source?.data?.src || '');
  return /^(data:image\/|blob:|https?:\/\/)/i.test(src);
}
function textureLooksBroken(tex) {
  if (!tex) return false;
  // BUILD141: a just-created ImageTexture from a data/blob URL can report
  // naturalWidth=0 before the browser finishes decoding. The BUILD138 HTML did
  // not delete those texture refs; deleting them here caused plain white CAD
  // materials even though the texture path was resolved correctly.
...
async function loadMeshObject(filename, resolver, fallbackMaterial) {
  ...
  try {
    if (ext === '.dae') {
      // BUILD141: use the same loading path as the repaired standalone HTML:
      // ColladaLoader.load(url) + LoadingManager URL rewrite. Do NOT manually
      // parse the DAE text and do NOT multiply by the <unit meter=...> value;
      // ColladaLoader already applies the asset unit. The previous modular build
      // double-scaled many Inventor/ROS Collada meshes and assembled robots with
      ...
      const collada = await loaderLoad(loader, daeUrl);
