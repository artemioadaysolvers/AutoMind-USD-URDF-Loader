...
class URDFPlusAssetResolver {
  ...
  add(key, val) {
    ...
    if (!data) return;
    for (const k of variantsFor(key)) if (!this.byKey.has(k)) this.byKey.set(k, data);
  }
  _lookup(path) {
    if (!path) return '';
    for (const k of variantsFor(path)) {
      const v = this.byKey.get(k);
      if (v) return v;
    }
  ...
  resolve(path) {
    ...
    // ColladaLoader, when the .dae itself is loaded from a data:model/... URL,
    // resolves relative textures as pseudo URLs like data:model/base.jpg. Those
    // are NOT real data URLs and must be redirected back to the uploaded assetDB.
    if (/^data:model\//i.test(raw)) {
      const tail = raw.replace(/^data:model\//i, '').replace(/[?#].*$/, '');
      return this._lookup(tail) || this._lookup(basename(tail)) || '';
    }

    // Real data URLs are already usable. Blob URLs created by the browser are
    // usable only as exact URLs; if a loader appends a relative texture path to a
    // blob/data base URL, fall through to basename lookup before giving up.
    if (/^data:[^,]+,/i.test(raw)) return raw;
    if (/^blob:/i.test(raw) || /^https?:\/\//i.test(raw)) {
      const byBase = this._lookup(basename(raw));
      return byBase || raw;
    }

    return this._lookup(raw);
  }
  dispose() {
