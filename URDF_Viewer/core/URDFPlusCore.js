...
// AutoMind URDF+ loader for the modular USD-style viewer shell. BUILD146: exact BUILD138 visual-tree/DAE/up-axis/material policy + Collada base64 textures; modern Three.js r164 compatible.
...
function normalizeUpAxisHint(value) {
  ...
  if (!s) return '';
  if (/^[+-][XYZ]$/.test(s)) return s;
  if (/^(X|Y|Z)_?UP$/.test(s)) return '+' + s[0];
  if (/^NEGATIVE[XYZ]$/.test(s)) return '-' + s.slice(-1);
  if (/^MINUS[XYZ]$/.test(s)) return '-' + s.slice(-1);
  if (/^[XYZ]$/.test(s)) return '+' + s;
  return '';
}
function detectPreferredUpAxis(robotNode, visualRobotNode, policy = {}, opts = {}) {
  const candidates = [
    opts.modelUpAxis, opts.cadUpAxis, opts.upAxis,
    policy?.cadUpAxis, policy?.modelUpAxis, policy?.upAxis
  ];
  ...
  for (const v of candidates) {
    const n = normalizeUpAxisHint(v);
    if (n) return n;
  }
  // BUILD138 fallback: converter/ROS/Collada URDF assets are Z-up, while the viewer grid is Y-up.
  return '+Z';
}
function applyModelUpAxisLikeBUILD138(model, upAxis) {
  if (!model) return '';
  const up = normalizeUpAxisHint(upAxis) || '+Z';
  model.rotation.set(0, 0, 0);
  if (up === '+Z') model.rotation.x = -Math.PI / 2;
  else if (up === '-Z') model.rotation.x = Math.PI / 2;
  else if (up === '+X') model.rotation.z = Math.PI / 2;
  else if (up === '-X') model.rotation.z = -Math.PI / 2;
  else if (up === '-Y') model.rotation.z = Math.PI;
  model.userData.__automindUpAxis = up;
  model.updateMatrixWorld(true);
  return up;
}
function materialIsWhiteBlackOrTooDark(mat) {
  if (!mat) return true;
  const c = mat.color;
  if (!c || typeof c.getHex !== 'function') return !materialHasUsefulTexture(mat);
  const hex = c.getHex();
  if (hex === 0xffffff || hex === 0x000000) return true;
  if (typeof c.getHSL === 'function') {
    const hsl = { h: 0, s: 0, l: 0 };
    try { c.getHSL(hsl); if (hsl.l < 0.045) return true; } catch (_) {}
  }
  return false;
}
function colorizeMissingMaterialsLikeBUILD138(model) {
  if (!model) return 0;
  let i = 0, changed = 0;
  const palette = [0xb9c7d6, 0x8ecae6, 0xcdb4db, 0xa7c957, 0xf6bd60, 0x90dbf4, 0xf1faee];
  model.traverse?.((obj) => {
    if (!obj?.isMesh) return;
    if (!obj.material) obj.material = createDefaultMaterial(palette[i % palette.length]);
    ...
    for (const mat of mats) {
      ...
      try {
        ...
        // Same readability rule as the standalone HTML: if a DAE arrives with a flat
        // white/black/no-map material, give it a stable CAD inspection color by mesh.
        if (!materialHasUsefulTexture(mat) && materialIsWhiteBlackOrTooDark(mat)) {
  ...
  model.userData.__automindMaterialColorized = changed;
  return changed;
}
...
function setObjectUserDataRecursive(obj, linkName) {
  obj?.traverse?.((o) => {
    ...
    if (o.isMesh) {
      ...
      for (const m of mats) {
        if (!m) continue;
        // BUILD146: keep pure white until colorizeMissingMaterialsLikeBUILD138() can assign the standalone palette.
...
function loadClassicScriptOnce(src, timeoutMs = 15000) {
  return new Promise((resolve, reject) => {
    ...
    script.src = src;
    script.async = true;
    // BUILD146: do not set crossOrigin; Colab can reject classic loader scripts when CORS is forced.
...
export async function loadURDFPlusModel(opts = {}) {
  ...
  const preferredUpAxis = detectPreferredUpAxis(robotNode, visualRobotNode, model.viewerPolicy, opts);
  applyModelUpAxisLikeBUILD138(model, preferredUpAxis);
  autoNormalizeDisplayScale(model);
  colorizeMissingMaterialsLikeBUILD138(model);
  model.updateMatrixWorld(true);
