...
// AutoMind URDF+ loader for the modular USD-style viewer shell. BUILD153: exact standalone HTML joint drag/loop solver + BUILD138 DAE root normalization + up-axis + material fallback; Collada textures inline base64 for Colab/jsDelivr.
...
function resolveColladaTextureAsDataURL(ref, resolver) {
  ...
  for (const c of colladaTextureCandidates(ref)) {
    ...
    if (/^data:image\//i.test(hit)) return hit;
    // A texture stored under an extensionless alias may come back as octet-stream.
    // Do not hand that to ImageLoader; fall through to a safe PNG placeholder.
  }
...
async function loadMeshObject(filename, resolver, fallbackMaterial) {
  ...
  try {
    if (ext === '.dae') {
      // BUILD145: transfer Collada image references as inline base64 data URLs.
      // The previous jsDelivr/Colab path let ColladaLoader derive fake URLs such
      // as blob:https://colab.../base.jpg from a DAE Blob base path, producing
      // net::ERR_FILE_NOT_FOUND even when the texture existed in assetDB.
      // Parsing the DAE XML directly after rewriting <init_from> avoids every
      // fake blob texture request and keeps ColladaLoader's own unit handling.
      ...
      setTimeout(() => { try { sanitizeLoadedMeshMaterials(obj, fallbackMaterial); } catch (_) {} }, 900);
      return obj;
    }
    if (ext === '.stl') {
      ...
      if (raw) {
        const s = String(raw || '');
        if (/^data:/i.test(s) || /^(blob:|https?:\/\/)/i.test(s)) geom = await loaderLoad(loader, resolver.resolve(filename));
        else {
...
class URDFPlusModel extends THREE.Group {
  ...
  applyPose() { solvePoseForCurrentValuesURDFPlus(this, true); }
  ...
  setJointValue(nameOrJoint, value) {
    ...
    // BUILD153: exact standalone HTML drag contract. During pointer-drag the
    // value is not applied blindly and corrected later; it is projected through
    // couplings + closed-chain solver in realtime, and a bisection limit keeps
    // the mechanism at the farthest feasible pose without snap-back on release.
    if (this.isDraggingJoint && !this.isSolvingLoops) {
      this.activeJointForDrag = this.activeJointForDrag || joint;
      applyConstrainedJointDragURDFPlus(this, joint, v);
      return;
    }
  ...
  getManipulableJointForLinkName(linkName) {
    ...
    // BUILD153: same priority as the standalone HTML. The direct child joint owns
    // the component when it is declared direct/authoritative/spin_only/self_only;
    // otherwise dependent joints can redirect to their coupling master, and fixed
    // links fall back to the nearest movable ancestor.
    const direct = this.parentJointByLink.get(linkName) || null;
    const directIsAuthoritative = direct && direct.movable && (
      this.viewerPolicy?.directChildJointFirst ||
      direct.interactiveControl === 'direct' ||
      direct.authoritativeKinematics ||
      direct.spinOnly ||
      direct.selfOnly
    );
    if (directIsAuthoritative) { this.manipulableJointByLink.set(linkName, direct); return direct; }

    if (direct && direct.movable) {
      const forceDirect = this.viewerPolicy?.disableCouplingRedirection || direct.interactiveControl === 'direct' || direct.spinOnly || direct.selfOnly;
      const master = forceDirect ? null : getMasterForDependentJointURDFPlus(this, direct);
      const chosen = master || direct;
      this.manipulableJointByLink.set(linkName, chosen);
      return chosen;
    }
    ...
    while (cur) {
      ...
      if (!j || seen.has(j.name)) break;
      seen.add(j.name);
      if (j.movable) {
        const master = this.viewerPolicy?.disableCouplingRedirection ? null : getMasterForDependentJointURDFPlus(this, j);
        const chosen = master || j;
        this.manipulableJointByLink.set(linkName, chosen);
        return chosen;
      }
      cur = j.parent;
  ...
  endInteractiveDrag(joint = null) {
    ...
    // Same as the HTML: final settle relaxes only passive/dependent joints while
    // keeping the user's joint pinned, so releasing the mouse preserves the pose.
    for (let i = 0; i < URDF_PLUS_REALTIME_DRAG_SOLVER.FINAL_SETTLE_PASSES; i++) {
      solvePoseForCurrentValuesURDFPlus(this, true);
    }
    this.pinnedSolverJointName = '';
  }
}
...
const URDF_PLUS_REALTIME_DRAG_SOLVER = {
  // Copied from the standalone HTML mechanism.
  DRAG_BINARY_STEPS: 8,
  DRAG_FEASIBLE_RESIDUAL: 1.5e-3,
  DRAG_FEASIBLE_RELATIVE: 2.75,
  FINAL_SETTLE_PASSES: 2,
  DLS_INTERACTIVE_MAX_ITER: 8,
  DLS_FINAL_MAX_ITER: 16,
  DLS_INTERACTIVE_MAX_VARS: 12,
  DLS_FINAL_MAX_VARS: 24,
  DLS_INTERACTIVE_MAX_CONSTRAINTS: 16,
  DLS_FINAL_MAX_CONSTRAINTS: 32,
  DLS_INTERACTIVE_LAMBDA: 1e-4,
  DLS_FINAL_LAMBDA: 5e-5,
  DLS_INTERACTIVE_RELAXATION: 0.85,
  DLS_FINAL_RELAXATION: 1.0,
  DLS_INTERACTIVE_STOP_RESIDUAL: 6e-4,
  DLS_FINAL_STOP_RESIDUAL: 1e-4
};
...
function getObjectBoundsURDFPlus(object, pad = 1.0) {
  const box = new THREE.Box3().setFromObject(object);
  if (box.isEmpty()) return null;
  const size = box.getSize(new THREE.Vector3()).multiplyScalar(pad || 1);
  return {
    box,
    size,
    center: box.getCenter(new THREE.Vector3()),
    radius: Math.max(size.length() * 0.5, 1e-9),
    maxDim: Math.max(size.x, size.y, size.z, 1e-9)
  };
}
function getMasterForDependentJointURDFPlus(model, joint) {
  if (!model || !joint) return null;
  const name = joint.name || '';
  if (!name) return null;
  for (const c of model.couplings || []) {
    const dep = c.dependent || c.dependentJoint || c.slave || c.target || '';
    const master = c.driver || c.masterJoint || c.master || c.joint || '';
    if (!dep || !master || dep !== name) continue;
    const m = model.joints?.[master];
    if (m && m.movable) return m;
  }
  const mimicMaster = joint.mimicJoint || joint.mimic || '';
  if (mimicMaster && model.joints?.[mimicMaster]?.movable) return model.joints[mimicMaster];
  return null;
}
function jointIsPrismaticURDFPlus(joint) {
  return /prismatic|slider/i.test(String(joint?.jointType || joint?.type || joint?.schema || ''));
}
function jointValueURDFPlus(joint) {
  return Number(joint?.value ?? (jointIsPrismaticURDFPlus(joint) ? joint?.position : joint?.angle) ?? 0) || 0;
}
function isMovableFullURDFPlus(joint) {
  return !!(joint && joint.tree !== false && joint.role !== 'loop' && joint.movable && joint.motionGroup);
}
function setJointScalarRawURDFPlus(model, joint, value, applyCouplings = false) {
  if (!joint || !joint.movable) return;
  const v = model._clampJointValue ? model._clampJointValue(joint, value) : Number(value || 0);
  model._setJointScalar(joint, v, !!applyCouplings);
}
function updateMatricesRawURDFPlus(model) {
  try { model.updateMatrixWorld(true); } catch (_) {}
  try { model.updateCurrentMatrices(); } catch (_) {}
}
function loopPointWorldURDFPlus(model, linkName, localPos, out = new THREE.Vector3()) {
  const info = model?._linkInfo?.[linkName];
  const p = Array.isArray(localPos) ? localPos : [0, 0, 0];
  out.set(Number(p[0]) || 0, Number(p[1]) || 0, Number(p[2]) || 0);
  if (info?.currentMatrix) out.applyMatrix4(info.currentMatrix);
  else model?.links?.[linkName]?.localToWorld?.(out);
  return out;
}
function loopAxisWorldURDFPlus(model, linkName, localAxis, out = new THREE.Vector3()) {
  const info = model?._linkInfo?.[linkName];
  const a = Array.isArray(localAxis) ? localAxis : [1, 0, 0];
  out.set(Number(a[0]) || 0, Number(a[1]) || 0, Number(a[2]) || 0);
  if (out.lengthSq() < EPS) out.set(1, 0, 0);
  if (info?.currentMatrix) out.transformDirection(info.currentMatrix);
  return out.normalize();
}
function isDescendantLinkOfJointURDFPlus(model, joint, linkName) {
  if (!model || !joint || !linkName) return false;
  const root = joint.child || joint.body1;
  let cur = linkName;
  const seen = new Set();
  while (cur && !seen.has(cur)) {
    if (cur === root) return true;
    seen.add(cur);
    const pj = model.parentJointByLink?.get?.(cur) || model._linkInfo?.[cur]?.parentJoint;
    if (!pj) break;
    cur = pj.parent || pj.body0;
  }
  return false;
}
function jointAffectsClosureURDFPlus(model, joint, loop) {
  if (!joint || !loop) return false;
  return isDescendantLinkOfJointURDFPlus(model, joint, loop.body0 || loop.predecessor) ||
         isDescendantLinkOfJointURDFPlus(model, joint, loop.body1 || loop.successor);
}
function activeSolverLoopsURDFPlus(model, maxConstraints = Infinity) {
  const raw = Array.isArray(model?.loopJoints) ? model.loopJoints : [];
  const out = [];
  const seen = new Set();
  for (const lp of raw) {
    const a = lp?.body0 || lp?.predecessor || lp?.parent;
    const b = lp?.body1 || lp?.successor || lp?.child;
    if (!a || !b || a === b || !model?._linkInfo?.[a] || !model?._linkInfo?.[b]) continue;
    const p0 = lp.localPos0 || lp.origin?.xyz || [0, 0, 0];
    const p1 = lp.localPos1 || lp.successorOrigin?.xyz || [0, 0, 0];
    const key = [a, b, (p0 || []).join(','), (p1 || []).join(',')].join('|');
    if (seen.has(key)) continue;
    seen.add(key);
    out.push(lp);
    if (out.length >= maxConstraints) break;
  }
  return out;
}
function loopsAffectedByJointURDFPlus(model, joint) {
  const loops = activeSolverLoopsURDFPlus(model, URDF_PLUS_REALTIME_DRAG_SOLVER.DLS_INTERACTIVE_MAX_CONSTRAINTS);
  const affected = loops.filter(lp => jointAffectsClosureURDFPlus(model, joint, lp));
  return affected.length ? affected : loops;
}
function closureConstraintCountURDFPlus(model) {
  return activeSolverLoopsURDFPlus(model, Infinity).length;
}
function collectLoopErrorURDFPlus(model, loops = null) {
  const list = loops || activeSolverLoopsURDFPlus(model, model.isDraggingJoint ? URDF_PLUS_REALTIME_DRAG_SOLVER.DLS_INTERACTIVE_MAX_CONSTRAINTS : URDF_PLUS_REALTIME_DRAG_SOLVER.DLS_FINAL_MAX_CONSTRAINTS);
  const e = [];
  updateMatricesRawURDFPlus(model);
  const pa = new THREE.Vector3(), pb = new THREE.Vector3();
  const aa = new THREE.Vector3(), ab = new THREE.Vector3(), cross = new THREE.Vector3();
  const b = getObjectBoundsURDFPlus(model, 1.0);
  const scale = Math.max(b?.maxDim || 1, 1e-4);
  const axisWeight = Math.min(Math.max(scale * 0.35, 1e-4), 0.05);
  for (const lp of list || []) {
    const a = lp.body0 || lp.predecessor || lp.parent;
    const c = lp.body1 || lp.successor || lp.child;
    loopPointWorldURDFPlus(model, a, lp.localPos0 || lp.origin?.xyz || [0,0,0], pa);
    loopPointWorldURDFPlus(model, c, lp.localPos1 || lp.successorOrigin?.xyz || [0,0,0], pb);
    e.push(pb.x - pa.x, pb.y - pa.y, pb.z - pa.z);
    const ax0 = lp.axisJoint || lp.axis || [1,0,0];
    const ax1 = lp.axisSuccessor || lp.axis_joint_successor || ax0;
    loopAxisWorldURDFPlus(model, a, ax0, aa);
    loopAxisWorldURDFPlus(model, c, ax1, ab);
    cross.crossVectors(aa, ab);
    e.push(cross.x * axisWeight, cross.y * axisWeight, cross.z * axisWeight);
  }
  return e;
}
function vectorNormURDFPlus(v) {
  if (!v || !v.length) return 0;
  let s = 0;
  for (const x of v) s += x * x;
  return Math.sqrt(s / Math.max(1, v.length / 3));
}
function candidateDependentJointsForLoopsURDFPlus(model, loops, changedJoint) {
  const all = Object.values(model?.joints || {}).filter(j => isMovableFullURDFPlus(j));
  const blocked = new Set();
  if (changedJoint?.name) blocked.add(changedJoint.name);
  if (model?.activeJointForDrag?.name) blocked.add(model.activeJointForDrag.name);
  if (model?.pinnedSolverJointName) blocked.add(model.pinnedSolverJointName);

  const affects = j => (loops || []).some(lp => jointAffectsClosureURDFPlus(model, j, lp));
  const eligible = all.filter(j => !blocked.has(j.name) && affects(j));
  const passive = eligible.filter(j => {
    if (j.independent === false || String(j.independent).toLowerCase() === 'false') return true;
    const role = String(j.kinematicRole || j.role || j.interactiveControl || '');
    if (/passive|dependent|driven|loop|follower/i.test(role)) return true;
    if (j.directUserControl === false || j.nonDirectAnimationCandidate) return true;
    return false;
  });
  const vars = passive.length ? passive : eligible;
  const max = model?.isDraggingJoint ? URDF_PLUS_REALTIME_DRAG_SOLVER.DLS_INTERACTIVE_MAX_VARS : URDF_PLUS_REALTIME_DRAG_SOLVER.DLS_FINAL_MAX_VARS;
  return vars.slice(0, max);
}
function solveDenseLinearSystemURDFPlus(A, b) {
  const n = A.length;
  if (!n) return [];
...
function solveLoopClosureForChangedJointURDFPlus(model, changedJoint = null) {
  const loops = changedJoint ? loopsAffectedByJointURDFPlus(model, changedJoint) : activeSolverLoopsURDFPlus(model);
  if (!loops.length) return;
  const vars = candidateDependentJointsForLoopsURDFPlus(model, loops, changedJoint);
  if (!vars.length) {
    model.lastLoopSolve = { residual: vectorNormURDFPlus(collectLoopErrorURDFPlus(model, loops)), vars: 0, iterations: 0, constraints: loops.length };
    return;
  }
  model.isSolvingLoops = true;
  try {
    let residual = Infinity;
    let lambda = model.isDraggingJoint ? URDF_PLUS_REALTIME_DRAG_SOLVER.DLS_INTERACTIVE_LAMBDA : URDF_PLUS_REALTIME_DRAG_SOLVER.DLS_FINAL_LAMBDA;
    const maxIter = model.isDraggingJoint ? URDF_PLUS_REALTIME_DRAG_SOLVER.DLS_INTERACTIVE_MAX_ITER : URDF_PLUS_REALTIME_DRAG_SOLVER.DLS_FINAL_MAX_ITER;
    const stopResidual = model.isDraggingJoint ? URDF_PLUS_REALTIME_DRAG_SOLVER.DLS_INTERACTIVE_STOP_RESIDUAL : URDF_PLUS_REALTIME_DRAG_SOLVER.DLS_FINAL_STOP_RESIDUAL;
    const relaxation = model.isDraggingJoint ? URDF_PLUS_REALTIME_DRAG_SOLVER.DLS_INTERACTIVE_RELAXATION : URDF_PLUS_REALTIME_DRAG_SOLVER.DLS_FINAL_RELAXATION;
    for (let it = 0; it < maxIter; it++) {
      updateMatricesRawURDFPlus(model);
      const e0 = collectLoopErrorURDFPlus(model, loops);
      residual = vectorNormURDFPlus(e0);
      if (residual < stopResidual) {
        model.lastLoopSolve = { residual, vars: vars.length, iterations: it, constraints: loops.length };
        break;
      }
      const m = e0.length, n = vars.length;
      const J = Array.from({ length: m }, () => Array(n).fill(0));
      for (let c = 0; c < n; c++) {
        const j = vars[c], old = jointValueURDFPlus(j), eps = jointIsPrismaticURDFPlus(j) ? 1e-5 : 1e-4;
        setJointScalarRawURDFPlus(model, j, old + eps, false);
        updateMatricesRawURDFPlus(model);
        const e1 = collectLoopErrorURDFPlus(model, loops);
        setJointScalarRawURDFPlus(model, j, old, false);
        for (let r = 0; r < m; r++) J[r][c] = ((e1[r] || 0) - (e0[r] || 0)) / eps;
      }
      ...
      for (let r = 0; r < m; r++) {
        for (let c = 0; c < n; c++) {
          rhs[c] -= J[r][c] * (e0[r] || 0);
          for (let k = 0; k < n; k++) A[c][k] += J[r][c] * J[r][k];
        }
      }
      for (let i = 0; i < n; i++) A[i][i] += lambda;
      const oldVals = vars.map(j => jointValueURDFPlus(j));
      const dq = solveDenseLinearSystemURDFPlus(A, rhs);
      let maxStep = 0;
      for (let c = 0; c < vars.length; c++) {
        const j = vars[c];
        let step = Number(dq[c] || 0) * relaxation;
        const lim = jointIsPrismaticURDFPlus(j) ? 0.004 : (model.isDraggingJoint ? 0.08 : 0.12);
        step = Math.max(-lim, Math.min(lim, step));
        setJointScalarRawURDFPlus(model, j, oldVals[c] + step, false);
        maxStep = Math.max(maxStep, Math.abs(step));
      }
      updateMatricesRawURDFPlus(model);
      const nextResidual = vectorNormURDFPlus(collectLoopErrorURDFPlus(model, loops));
      if (nextResidual > residual * 1.15) {
        for (let c = 0; c < vars.length; c++) setJointScalarRawURDFPlus(model, vars[c], oldVals[c], false);
        lambda *= 10;
      } else {
        residual = nextResidual;
        lambda = Math.max(lambda * 0.6, 1e-7);
      }
      model.lastLoopSolve = { residual, vars: vars.length, iterations: it + 1, constraints: loops.length };
      if (maxStep < 5e-8) break;
    }
  } finally {
    model.isSolvingLoops = false;
  }
}
function propagateCouplingsForSolverURDFPlus(model, masterJoint) {
  if (!model || !masterJoint || model.__applyingCouplings) return;
  model._propagateCouplingsFrom(masterJoint);
}
function solvePoseForCurrentValuesURDFPlus(model, updateWorld = true, changedJoint = null) {
  if (!model) return;
  const driver = changedJoint || model.activeJointForDrag || null;
  propagateCouplingsForSolverURDFPlus(model, driver);
  updateMatricesRawURDFPlus(model);
  if (!model.isSolvingLoops) solveLoopClosureForChangedJointURDFPlus(model, driver);
  propagateCouplingsForSolverURDFPlus(model, driver);
  updateMatricesRawURDFPlus(model);
  if (updateWorld) updateMatricesRawURDFPlus(model);
}
function snapshotMovableJointValuesURDFPlus(model) {
  return Object.values(model?.joints || {}).filter(j => isMovableFullURDFPlus(j)).map(j => ({ joint: j, value: jointValueURDFPlus(j) }));
}
function restoreMovableJointValuesURDFPlus(model, snapshot) {
  for (const s of snapshot || []) if (s?.joint) setJointScalarRawURDFPlus(model, s.joint, s.value || 0, false);
  updateMatricesRawURDFPlus(model);
}
function currentSolverResidualForJointURDFPlus(model, joint) {
  const loops = joint ? loopsAffectedByJointURDFPlus(model, joint) : activeSolverLoopsURDFPlus(model);
  if (!loops.length) return 0;
  const residual = vectorNormURDFPlus(collectLoopErrorURDFPlus(model, loops));
  model.lastLoopSolve = {
    residual,
    vars: Math.max(0, candidateDependentJointsForLoopsURDFPlus(model, loops, joint).length),
    iterations: model.lastLoopSolve?.iterations || 0,
    constraints: loops.length
  };
  return residual;
}
function dragFeasibleResidualLimitURDFPlus(baseResidual) {
  const b = Number.isFinite(baseResidual) ? Math.max(0, baseResidual) : 0;
  return Math.max(URDF_PLUS_REALTIME_DRAG_SOLVER.DRAG_FEASIBLE_RESIDUAL, b * URDF_PLUS_REALTIME_DRAG_SOLVER.DRAG_FEASIBLE_RELATIVE);
}
function attemptConstrainedDragValueURDFPlus(model, joint, value, baseSnapshot) {
  restoreMovableJointValuesURDFPlus(model, baseSnapshot);
  setJointScalarRawURDFPlus(model, joint, value, false);
  solvePoseForCurrentValuesURDFPlus(model, false, joint);
  return {
    residual: currentSolverResidualForJointURDFPlus(model, joint),
    value: jointValueURDFPlus(joint),
    snapshot: snapshotMovableJointValuesURDFPlus(model)
  };
}
function applyConstrainedJointDragURDFPlus(model, joint, targetValue) {
  if (!model || !joint) return false;
  const startValue = jointValueURDFPlus(joint);
  const baseSnapshot = snapshotMovableJointValuesURDFPlus(model);
  const baseResidual = currentSolverResidualForJointURDFPlus(model, joint);
  const residualLimit = dragFeasibleResidualLimitURDFPlus(baseResidual);

  const direct = attemptConstrainedDragValueURDFPlus(model, joint, targetValue, baseSnapshot);
  if (!closureConstraintCountURDFPlus(model) || direct.residual <= residualLimit) {
    restoreMovableJointValuesURDFPlus(model, direct.snapshot);
    solvePoseForCurrentValuesURDFPlus(model, true, joint);
    return true;
  }

  let lo = 0.0, hi = 1.0;
  let best = null;
  for (let i = 0; i < URDF_PLUS_REALTIME_DRAG_SOLVER.DRAG_BINARY_STEPS; i++) {
    const mid = (lo + hi) * 0.5;
    const v = startValue + (targetValue - startValue) * mid;
    const trial = attemptConstrainedDragValueURDFPlus(model, joint, v, baseSnapshot);
    if (trial.residual <= residualLimit) { best = trial; lo = mid; }
    else hi = mid;
  }
  if (best) restoreMovableJointValuesURDFPlus(model, best.snapshot);
  else {
    restoreMovableJointValuesURDFPlus(model, baseSnapshot);
    setJointScalarRawURDFPlus(model, joint, startValue, false);
  }
  solvePoseForCurrentValuesURDFPlus(model, true, joint);
  return !!best;
}
...
export async function loadURDFPlusModel(opts = {}) {
  ...
  model.manipulableJointByLink.clear();
  model.applyPose();
  return model;
}
