...
// AutoMind URDF+ loader for the modular USD-style viewer shell. BUILD139: standalone BUILD138 URDF mechanism merged into modular viewer; null vectors, package zip meshes, and file texture fallbacks.
...
function mergeUrdfPlusMetaIntoJoint(joint, metaIndex) {
  ...
  // BUILD140: the standalone HTML treats URDF+ authoritative axis/origin as the
...
function findVisualURDFForMain(mainText, assetDB = {}, opts = {}) {
  ...
  if (backups.length && (!mainSafe || policy.preferStandardBackup !== false || opts.forceVisualBackup === true)) {
...
class URDFPlusAssetResolver {
  ...
  resolveForLoader(path) {
    ...
    const hit = this.resolve(raw) || this.resolve(cleanPath(raw)) || this.resolve(basename(raw));
    if (hit) return hit;
    if (isRealExternalUrl(raw)) return raw;
    // BUILD138/139: local file:// and broken Collada data:model/*.jpg texture
    // paths cannot be fetched from browser sandboxes. Return a valid transparent
    // texture so the mesh still loads and materials are sanitized afterward.
    if (/^file:/i.test(raw) || isTextureLikePath(raw) || /^data:/i.test(raw)) return TRANSPARENT_PNG_DATA_URL;
    return cleanPath(raw) || raw;
  }
...
function textureLooksBroken(tex) {
  if (!tex) return false;
  // A Collada material can keep a Texture object even when the image never loaded.
  // That is the typical cause of the all-black silhouette in Colab/jsDelivr.
  return !textureHasDecodedImage(tex) && !tex.isDataTexture;
}
...
async function loadMeshObject(filename, resolver, fallbackMaterial) {
  ...
  try {
    if (ext === '.dae') {
      // Use the original AutoMind URDF loader strategy for DAE: parse the DAE text
      // locally and let a LoadingManager rewrite every texture/subresource request
      // back into assetDB data URLs. This avoids the Colab/jsDelivr data-URL edge
      // case that produced black silhouettes and broken thumbnails.
      const mgr = new THREE.LoadingManager();
      mgr.setURLModifier((u) => resolver.resolveForLoader(u));
      const loader = new THREE.ColladaLoader(mgr);
      let obj = null;
      const rawText = resolver.getRawText(filename);
      let daeUnitScale = 1.0;
      if (rawText && /<COLLADA\b/i.test(rawText)) {
        const um = /<unit[^>]*meter\s*=\s*["']([\d.eE+\-]+)["']/i.exec(rawText);
        if (um) {
          const meter = parseFloat(um[1]);
          if (Number.isFinite(meter) && meter > 0) daeUnitScale = meter;
        }
        const baseDir = cleanPath(filename).replace(/[^/]*$/, '');
        const collada = loader.parse(rawText, baseDir);
        obj = collada.scene || collada;
        await waitForLoadingManager(mgr, 5500);
      } else {
        const collada = await loaderLoad(loader, url);
        obj = collada.scene || collada;
        await waitForLoadingManager(mgr, 3500);
      }
      if (daeUnitScale !== 1.0 && obj?.scale) obj.scale.multiplyScalar(daeUnitScale);
      sanitizeLoadedMeshMaterials(obj, fallbackMaterial);
      // One additional delayed pass catches Texture objects whose image failed
      // after ColladaLoader created materials but before the first render.
      setTimeout(() => { try { sanitizeLoadedMeshMaterials(obj, fallbackMaterial); } catch (_) {} }, 700);
      return obj;
    }
    if (ext === '.stl') {
...
class URDFPlusModel extends THREE.Group {
  ...
  applyPose() { solvePoseForCurrentValues(this, true); }
...
function jointValue(j) { return Number(j?.value ?? (/prismatic/i.test(j?.jointType || '') ? j?.position : j?.angle) ?? 0) || 0; }
function jointIsPrismatic(j) { return /prismatic/i.test(String(j?.jointType || j?.type || '')); }
function clampValueForJoint(j, v) {
  let val = Number(v); if (!Number.isFinite(val)) val = 0;
  const lo = Number.isFinite(j.lower) ? j.lower : (jointIsPrismatic(j) ? -1 : -Math.PI * 2);
  const hi = Number.isFinite(j.upper) ? j.upper : (jointIsPrismatic(j) ? 1 : Math.PI * 2);
  return Math.max(lo, Math.min(hi, val));
}
function rawSetJointScalar(j, v) {
  if (!j) return;
  const val = clampValueForJoint(j, v);
  j.value = val;
  if (jointIsPrismatic(j)) j.position = val; else j.angle = val;
  applyJointMotion(j);
}
function activeLoopJoints(model) {
  return (model?.loopJoints || []).filter(l => {
    const a = l.body0 || l.predecessor;
    const b = l.body1 || l.successor;
    return a && b && a !== b && model.links?.[a] && model.links?.[b];
  });
}
function updateModelMatrices(model) {
  model?.updateMatrixWorld?.(true);
  model?.updateCurrentMatrices?.();
}
function worldLoopPoint(model, linkName, localPos) {
  const g = model.links?.[linkName];
  const p = Array.isArray(localPos) ? localPos : [0,0,0];
  return new THREE.Vector3(Number(p[0]) || 0, Number(p[1]) || 0, Number(p[2]) || 0).applyMatrix4(g?.matrixWorld || new THREE.Matrix4());
}
function collectLoopError(model) {
  const e = [];
  updateModelMatrices(model);
  for (const l of activeLoopJoints(model)) {
    const a = l.body0 || l.predecessor;
    const b = l.body1 || l.successor;
    const pa = worldLoopPoint(model, a, l.localPos0 || l.origin?.xyz || [0,0,0]);
    const pb = worldLoopPoint(model, b, l.localPos1 || l.successorOrigin?.xyz || [0,0,0]);
    e.push(pb.x - pa.x, pb.y - pa.y, pb.z - pa.z);
  }
  return e;
}
function normVec(vals) {
  if (!vals?.length) return 0;
  let s = 0; for (const v of vals) s += v * v;
  return Math.sqrt(s / Math.max(1, vals.length / 3));
}
function ancestorMovableJoints(model, linkName) {
  const out = [];
  const seen = new Set();
  let cur = linkName;
  while (cur && !seen.has(cur)) {
    seen.add(cur);
    const j = model.parentJointByLink?.get(cur);
    if (!j) break;
    if (j.movable) out.push(j);
    cur = j.parent;
  }
  return out;
}
function passiveLoopSolverJoints(model) {
  const names = new Set();
  for (const l of activeLoopJoints(model)) {
    for (const j of ancestorMovableJoints(model, l.body0 || l.predecessor)) names.add(j.name);
    for (const j of ancestorMovableJoints(model, l.body1 || l.successor)) names.add(j.name);
  }
  const vars = [];
  for (const name of names) {
    const j = model.joints?.[name];
    if (!j || !j.movable) continue;
    if (model.pinnedSolverJointName && j.name === model.pinnedSolverJointName) continue;
    if (model.activeJointForDrag && j.name === model.activeJointForDrag.name) continue;
    if (String(j.independent) === 'true' || j.independent === true) continue;
    vars.push(j);
  }
  return vars.slice(0, model.isDraggingJoint ? 48 : 72);
}
function solveDenseNormal(J, e, lambda = 1e-3) {
...
function solveLoopClosureDLS(model) {
  const loops = activeLoopJoints(model);
  if (!loops.length || model.isSolvingLoops) return;
  const vars = passiveLoopSolverJoints(model);
  if (!vars.length) { model.lastLoopSolve = { residual: normVec(collectLoopError(model)), vars: 0, iterations: 0, constraints: loops.length }; return; }
  model.isSolvingLoops = true;
  try {
    const maxIter = model.isDraggingJoint ? 6 : 12;
    let lambda = model.isDraggingJoint ? 1.2e-2 : 4e-3;
    const stop = model.isDraggingJoint ? 2.5e-4 : 3.5e-5;
    for (let it = 0; it < maxIter; it++) {
      const e0 = collectLoopError(model);
      const residual = normVec(e0);
      if (residual < stop) { model.lastLoopSolve = { residual, vars: vars.length, iterations: it, constraints: loops.length }; break; }
      const m = e0.length, n = vars.length;
      const J = Array.from({ length: m }, () => Array(n).fill(0));
      for (let c = 0; c < n; c++) {
        const j = vars[c];
        const old = jointValue(j);
        const eps = jointIsPrismatic(j) ? 1e-5 : 1e-4;
        rawSetJointScalar(j, old + eps);
        const e1 = collectLoopError(model);
        rawSetJointScalar(j, old);
        for (let r = 0; r < m; r++) J[r][c] = (e1[r] - e0[r]) / eps;
      }
      const oldVals = vars.map(jointValue);
      let dq = solveDenseNormal(J, e0, lambda);
      if (!dq) break;
      let maxStep = 0;
      for (let c = 0; c < vars.length; c++) {
        const j = vars[c];
        const lim = jointIsPrismatic(j) ? 0.004 : (model.isDraggingJoint ? 0.08 : 0.12);
        const step = Math.max(-lim, Math.min(lim, Number(dq[c] || 0) * (model.isDraggingJoint ? 0.52 : 0.72)));
        rawSetJointScalar(j, oldVals[c] + step);
        maxStep = Math.max(maxStep, Math.abs(step));
      }
      const nextResidual = normVec(collectLoopError(model));
      if (nextResidual > residual * 1.15) {
        for (let c = 0; c < vars.length; c++) rawSetJointScalar(vars[c], oldVals[c]);
        lambda *= 10;
      } else lambda = Math.max(lambda * 0.6, 1e-7);
      model.lastLoopSolve = { residual: nextResidual, vars: vars.length, iterations: it + 1, constraints: loops.length };
...
function solvePoseForCurrentValues(model, updateWorld = true) {
  ...
  if (updateWorld) updateModelMatrices(model);
}
...
function parseJoint(jointNode, model) {
  ...
  const j = {
    ...
    independent: boolAttr(attrAny(jointNode, ['independent','automind:independent'], evidenceNode?.getAttribute('independent') || ''), false),
