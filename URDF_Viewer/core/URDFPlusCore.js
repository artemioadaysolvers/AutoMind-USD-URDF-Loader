...
// AutoMind URDF+ loader for the modular USD-style viewer shell. BUILD159: paced DAE/base64 texture pipeline at 3x faster waits.
...
function automindSolidifyCadMaterial(mat) {
  ...
  try {
    ...
    if ('envMapIntensity' in mat && Number.isFinite(mat.envMapIntensity)) mat.envMapIntensity = Math.min(mat.envMapIntensity, 0.65);
    if ('roughness' in mat && Number.isFinite(mat.roughness)) mat.roughness = Math.max(0.38, Math.min(mat.roughness, 0.86));
    if ('metalness' in mat && Number.isFinite(mat.metalness)) mat.metalness = Math.min(mat.metalness, 0.22);
