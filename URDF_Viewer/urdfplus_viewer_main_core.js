...
// BUILD161: clean URDF+ viewer, no fog, uniform grid, camera tween focus, hard Show all reset.
...
// BUILD160: thumbnail waits are 4x shorter than BUILD159 while preserving the serial queue.
...
function applyRenderModeMaterialState(core, mat) {
  ...
  if (mat.map && mat.color) {
    // Texture colors are already in the image; do not multiply them by a pale CAD diffuse.
...
function showAll(core) {
  ...
  // BUILD161: Show all is a hard canonical render-mode restore.
  // No opacity tween here: the old tween temporarily made Solid look like Ghost/X-Ray,
  // which caused the white/fog layer and the visible render-mode flicker.
...
function buildThumbnailer(core, assetToMeshes) {
  ...
  async function waitForTextures(meshes, timeoutMs = 6000) {
    ...
    await Promise.all(maps.map(async (tex) => {
      ...
      if (img && typeof img.decode === 'function') {
        try { await Promise.race([img.decode(), sleep(Math.min(420, timeoutMs))]); } catch (_) {}
      }
    }));
  ...
  async function renderThumbnailNow(assetKey) {
    ...
    // BUILD160 anti-fog: previous thumbnail lights were heavily overexposed and
    // made textured CAD parts look milky/white. Use balanced neutral light.
    scene.add(new THREE.HemisphereLight(0xffffff, 0xdce8ee, 0.72));
    scene.add(new THREE.AmbientLight(0xffffff, 0.18));
    const key = new THREE.DirectionalLight(0xffffff, 0.92); key.position.set(3, 4, 5); scene.add(key);
    const fill = new THREE.DirectionalLight(0xffffff, 0.22); fill.position.set(-4, -2, 3); scene.add(fill);
    ...
    try {
      ...
      if (typeof url === 'string' && /^data:image\/png;base64,/i.test(url) && url.length > 64) {
        ...
        else setTimeout(() => { try { cache.delete(assetKey); } catch (_) {} }, 90);
        return url;
      }
      return '';
    } catch (e) {
  ...
  async function primeAll(keys) { for (const k of keys || []) { try { await thumbnail(k); await sleep(10); } catch (_) {} } }
...
export function render(opts = {}) {
  ...
  app.ready = (async () => {
    try {
      ...
      setTimeout(() => { try { thumbs.primeAll(Array.from(assetToMeshes.keys())); } catch (_) {} }, 200);
