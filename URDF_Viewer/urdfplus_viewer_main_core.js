...
import { loadURDFPlusModel } from './core/URDFPlusCore.js';
export let Base64Images = [];
function debugLog(...args) {
    const enabled = !!(globalThis?.AutoMindURDFPlusDebug || globalThis?.AUTOMIND_DEBUG);
    try {
        window.URDFPLUS_DEBUG_LOGS = window.URDFPLUS_DEBUG_LOGS || [];
        window.URDFPLUS_DEBUG_LOGS.push(args);
    }
    catch (_) { }
    if (!enabled)
        return;
    try {
        console.log('[URDFPLUS_DEBUG]', ...args);
    }
    catch (_) { }
}
...
function distanceToFitSphere(cam, radius, pad = 3) {
    ...
    if (cam.isOrthographicCamera)
        return THREE.MathUtils.clamp(r * 2.0, 0.35, 1e4);
...
function cloneMaterialForVisibility(mat) {
    ...
    if (!Number.isFinite(ud.__automindBaseOpacity))
        ud.__automindBaseOpacity = op > 0 ? op : 1;
    if (typeof ud.__automindBaseTransparent !== 'boolean')
        ud.__automindBaseTransparent = !!cloned.transparent;
    if (typeof ud.__automindBaseDepthWrite !== 'boolean')
        ud.__automindBaseDepthWrite = cloned.depthWrite !== false;
    if (typeof ud.__automindBaseDepthTest !== 'boolean')
        ud.__automindBaseDepthTest = cloned.depthTest !== false;
    return cloned;
}
...
function showAll(core) {
    ...
    try {
        core.setMechanismSuppressed?.(false, 520);
    }
    catch (_) { }
...
function isolateMeshesSmooth(core, meshesToKeep, duration = 560) {
    ...
    try {
        core.setMechanismSuppressed?.(kept.length > 0, duration);
    }
    catch (_) { }
...
function buildThumbnailer(core, assetToMeshes) {
    ...
    function textureLooksBroken(tex) { return !!(tex && !tex.isDataTexture && !hasLoadedImage(tex)); }
    ...
    async function waitForTextures(meshes, timeoutMs = 6500) {
        const maps = collectTextureMaps(meshes);
        if (!maps.length)
            return { maps, loaded: true };
        const t0 = performance.now();
        await new Promise(resolve => {
            const tick = () => {
                if (maps.every(t => t.isDataTexture || hasLoadedImage(t)) || performance.now() - t0 > timeoutMs)
                    resolve();
                else
                    setTimeout(tick, 35);
            };
    ...
    function cloneMaterialForPreview(src) {
        ...
        if (src && typeof src.clone === 'function')
            ...
            mat = new THREE.MeshStandardMaterial({ color: 0xdfe8ea, roughness: 0.55, metalness: 0.04 });
        for (const k of ['map', 'emissiveMap', 'aoMap', 'alphaMap', 'bumpMap', 'normalMap', 'roughnessMap', 'metalnessMap', 'specularMap']) {
            if (mat[k] && textureLooksBroken(mat[k]))
                mat[k] = null;
        }
        mat.side = THREE.DoubleSide;
        if (mat.color) {
            const c = mat.color;
            if (c.r > 0.92 && c.g > 0.92 && c.b > 0.92 && !mat.map)
                mat.color = new THREE.Color(0xe1e8ea);
            if (c.r < 0.030 && c.g < 0.030 && c.b < 0.030 && !mat.map)
                mat.color = new THREE.Color(0xdfe8ea);
        }
    ...
    function disposePreview(root) {
        root?.traverse?.((o) => {
            ...
            if (o?.isLineSegments && o.material)
                o.material.dispose?.();
            if (o?.userData?.__ownedEdgeGeometry)
                o.geometry?.dispose?.();
        });
    }
    ...
    async function thumbnail(assetKey) {
        ...
        try {
            const url = renderSceneToDataURL(scene, camera);
            if (typeof url === 'string' && /^data:image\/png;base64,/i.test(url) && url.length > 64) {
                if (!texStatus.maps.length || texStatus.loaded)
    ...
    async function primeAll(keys) { for (const k of keys || []) {
        try {
            await thumbnail(k);
        }
        catch (_) { }
    ...
    setTimeout(async () => {
        try {
            ...
            if (typeof cb !== 'function') {
                debugLog('[IA] Colab callback unavailable');
                return;
            }
            ...
            try {
                ...
                if (iso)
                    entries.push({ key: '__robot_iso__', name: 'robot_iso', index: -1, image_b64: iso.split(',')[1] || '' });
            }
            catch (_) { }
            for (let i = 0; i < items.length; i++) {
                ...
                if (b64)
                    entries.push({ key: it.assetKey, name: it.base || it.assetKey, index: i, image_b64: b64 });
            }
...
async function captureRobotISO(app) {
    const map = new Map();
    const arr = [];
    app.robot?.traverse?.(o => { if (o.isMesh && o.geometry)
        arr.push(o); });
...
export function render(opts = {}) {
    const { container, selectMode = 'link', background = (THEME.colors?.canvasBg ?? THEME.bgCanvas ?? 0xffffff), pixelRatio = Math.min(window.devicePixelRatio || 1, 2), IA_Widgets = false, } = opts;
    if (!container)
        throw new Error('[urdfplus_viewer_main] opts.container is required');
    ...
    const inter = attachInteraction({
        ...
        onSelectLink: () => { try {
            app?.interaction?.refreshSelectionMarker?.();
        }
        catch (_) { } }
    });
    ...
    app.ready = (async () => {
        try {
            ...
            try {
                core.setMechanismToggles?.({ jointAxes: false, loops: false });
            }
            catch (_) { }
            setTimeout(() => { try {
                thumbs.primeAll(Array.from(assetToMeshes.keys()));
            }
            catch (_) { } }, 1400);
            maybeSetupIA(app, assetToMeshes, thumbs);
            debugLog('URDF+ loaded BUILD147', {
                robot: robot.name,
