...
function debugLog(...args) {
    try {
        console.log('[USD_DEBUG]', ...args);
    }
    catch (_) { }
    try {
        window.USD_DEBUG_LOGS = window.USD_DEBUG_LOGS || [];
        window.USD_DEBUG_LOGS.push(args);
    }
    catch (_) { }
}
...
function buildThumbnailer(core, assetToMeshes) {
    ...
    async function waitForTextures(meshes, timeoutMs = 10000) {
        ...
        await new Promise(resolve => {
            const tick = () => {
                if (maps.every(hasLoadedImage) || performance.now() - t0 > timeoutMs)
                    resolve();
                else
                    setTimeout(tick, 35);
            };
...
export function render(opts = {}) {
    const { container, usdContent = '', usdText = '', assetDB = {}, meshDB = {}, textureDB = {}, selectMode = 'link', background = (THEME.colors?.canvasBg ?? THEME.bgCanvas ?? 0xffffff), pixelRatio = Math.min(window.devicePixelRatio || 1, 2), IA_Widgets = false, } = opts;
    if (!container)
        throw new Error('[usd_viewer_main] opts.container is required');
    ...
    if (typeof window !== 'undefined') {
        window.USDViewer = window.USDViewer || {};
        window.USDViewer.__app = app;
    }
