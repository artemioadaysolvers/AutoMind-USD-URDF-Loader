...
export function createComponentsPanel(app, theme) {
  ...
  async function renderList() {
    ...
    items.forEach((ent, index) => {
      ...
      const replaceThumbFallback = () => {
        if (thumbReplaced || !img.parentNode) return;
        thumbReplaced = true;
        img.replaceWith(makeThumbFallback(ent.base, theme));
      };
      img.addEventListener("error", replaceThumbFallback, { once: true });
  ...
  function showDetails(ent, index) {
    ...
    if (app.debug) console.debug("[ComponentsPanel] showDetails:", ent.assetKey, "=>", text);
    updateResponsiveLayout();
  }
  ...
  function onIAReady(ev) {
    if (app.debug) console.debug("[ComponentsPanel][IA] ia_descriptions_ready", ev && ev.detail);
  ...
  const pollTimer = setInterval(() => {
    ...
    if (
      ...
      if (app.debug) console.debug("[ComponentsPanel][IA] Descripciones detectadas por poll");
      refreshCurrentDetailsFromIA();
      clearInterval(pollTimer);
    }
