export function createToolsDock(app, theme) {
    ...
    const mkSelect = (options, value) => {
        ...
        function setOpen(next) {
            ...
            if (opened) {
                ...
                setTimeout(() => { if (!opened)
                    menu.style.display = 'none'; }, 150);
            }
        }
        ...
        face.addEventListener('mouseenter', () => { if (!opened)
            face.style.background = theme.tealFaint; });
        face.addEventListener('mouseleave', () => { if (!opened)
            face.style.background = `linear-gradient(180deg, ${theme.bgPanel}, #f6fbfb)`; });
        root.addEventListener('keydown', (ev) => {
            ...
            if (ev.key === 'ArrowDown') {
                ev.preventDefault();
                setValue(options[Math.min(options.length - 1, idx + 1)] ?? current, true);
            }
            if (ev.key === 'ArrowUp') {
                ev.preventDefault();
                setValue(options[Math.max(0, idx - 1)] ?? current, true);
            }
        });
        document.addEventListener('pointerdown', (ev) => { if (opened && !root.contains(ev.target))
            setOpen(false); }, true);
    ...
    rowCam.appendChild(bIso);
    rowCam.appendChild(bTop);
    rowCam.appendChild(bFront);
    rowCam.appendChild(bRight);
    ...
    function setRenderMode(mode) {
        try {
            app.setRenderModeState?.(mode);
        }
        catch (_) {
            app.__currentRenderMode = mode || 'Solid';
        }
    ...
    function traverseRobotMeshes(fn) {
        ...
        app.robot.traverse((o) => {
            if (!o || !o.isMesh || !o.geometry)
                return;
            if (o.userData && o.userData.__isHoverOverlay)
                return;
            fn(o);
        });
    }
    ...
    function refreshSectionVisual(maxDim, center) {
        ...
        if (center)
            secVisual.position.copy(center);
    }
    function updateSectionPlane() {
        ...
        if (!secEnabled || !app.robot) {
            sectionPlane = null;
            try {
                app.sectionPlane = null;
                app.getSectionPlane = () => null;
                app.setMechanismClippingPlane?.(null);
            }
            catch (_) { }
        ...
        sectionPlane = plane;
        try {
            app.sectionPlane = plane;
            app.getSectionPlane = () => sectionPlane;
        }
        catch (_) { }
        applyRobotOnlyClipping(plane);
        try {
            app.setMechanismClippingPlane?.(plane);
        }
        catch (_) { }
        ensureSectionVisual();
    ...
    function tweenOrbits(cam, ctrl, toPos, toTarget = null, ms = 700) {
        ...
        function step(t) {
            ...
            if (moveTarget)
                ctrl.target.set(t0.x + (toTarget.x - t0.x) * e, t0.y + (toTarget.y - t0.y) * e, t0.z + (toTarget.z - t0.z) * e);
            ctrl.update();
    ...
    function getActiveFitSphere(app_) {
        try {
            ...
            if (focused && focused.center && focused.size && Number.isFinite(focused.radius))
                return focused;
        }
        catch (_) { }
    ...
    function viewEndPose(kind) {
        ...
        if (kind === 'top') {
            az = Math.round(cur.az / (Math.PI / 2)) * (Math.PI / 2);
            el = Math.PI / 2 - topEps;
        }
        if (kind === 'front') {
            az = Math.PI / 2;
    ...
    setTimeout(() => {
        try {
            app.setMechanismToggles?.({ jointAxes: !!togShowJoints.cb.checked, loops: !!togShowLoops.cb.checked });
        }
        catch (_) { }
    }, 0);
    function makeExplodeManager() {
        ...
        function prepare() {
            ...
            parts.forEach((node) => {
                ...
                if (!isFinite(dirWorld.x + dirWorld.y + dirWorld.z))
                    return;
    ...
    try {
        app.explodeRecalibrate = () => explode.recalibrate();
    }
    catch (_) { }
    ...
    function syncExplodeUI() {
        try {
            ...
            if (!Number.isNaN(a))
                explodeSlider.value = String(Math.max(0, Math.min(1, a)));
        }
        catch { }
    }
