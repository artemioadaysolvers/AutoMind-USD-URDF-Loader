...
async function makeMaterial(def, assetDB, geomNode = null) {
  ...
  const keepDirectMtlMap = window.AutoMindMJCFForceDirectMTLMaps !== false;
  ...
  if (map) {
    ...
    mat = new THREE.MeshBasicMaterial({
      ...
      color: (useEmergencyFallback || !keepDirectMtlMap) ? emergencyColor : new THREE.Color(0xffffff),
      map: (useEmergencyFallback || !keepDirectMtlMap) ? null : map,
    ...
    mat.userData = {
      ...
      __automindMtlDiffuseMap: !useEmergencyFallback && keepDirectMtlMap
    };
  } else {
...
export async function loadMJCFModel(opts = {}) {
  ...
  model.userData.__automindBuild = 'MJCF BUILD183 — direct MTL diffuse-map rendering';
