...
class MJCFModel extends THREE.Group {
  ...
  _solveLoopClosures(changedJoint = null) {
    ...
    try {
      const maxIter = this.isDraggingJoint ? 8 : 16;
      const stopResidual = Math.max(this._modelScale() * 2e-4, this.isDraggingJoint ? 8e-5 : 2e-5);
      let lambda = this.isDraggingJoint ? 1e-3 : 1e-5;
      ...
      for (let iter = 0; iter < maxIter; iter++) {
        ...
        for (let c = 0; c < n; c++) {
          const j = variables[c];
          const old = j.value || 0;
          const eps = /slide/i.test(j.type || '') ? 1e-5 : 1e-5;
        ...
        for (let i = 0; i < n; i++) {
          ...
          for (let k = 0; k < n; k++) {
            let s = 0; for (let r = 0; r < m; r++) s += J[r][i] * J[r][k];
            A[i][k] = s;
          }
          A[i][i] += lambda;
        }
        ...
        if (maxAbs > 0.20) dq = dq.map(v => v * (0.20 / maxAbs));
        ...
        for (const alpha of [1, 0.5, 0.25, 0.125, 0.0625]) {
          ...
          if (Number.isFinite(trial) && trial + 1e-12 < residual) {
            residual = trial;
  ...
  applyPose(changedJoint = this._lastCommandJoint || null) {
    // Atomic kinematic transaction: first propagate the requested coordinate,
    // then solve closures from those current matrices, then propagate the final
    // coupled/solved state. This prevents loop anchors from being evaluated one
    // hierarchy step behind when a lower passive link is manipulated.
    this._applyCouplings();
    this._refreshLinkMatrices();
...
export async function loadMJCFModel(opts = {}) {
  ...
  model.userData.__automindBuild = 'MJCF BUILD212 — contextual axis validation with exact paired-site loop residuals';
