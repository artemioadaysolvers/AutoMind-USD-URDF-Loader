...
// BUILD220: MuJoCo-style committed qpos presentation sync + constrained closed-chain drag.
...
export function render(opts = {}) {
  ...
  debugLog('render init', {
    mode: 'MJCF BUILD220_MUJOCO_STYLE_KINEMATIC_COMMIT',
    selectMode,
    IA_Widgets,
  ...
  const inter = attachInteraction({
    ...
    onKinematicCommit: () => {
      try { core.__automindSyncPresentation?.(); } catch (_) {}
      try { core.syncKinematicPresentation?.(); } catch (_) {}
    }
  });
  ...
  app.ready = (async () => {
    try {
      ...
      debugLog('MJCF loaded BUILD220_MUJOCO_STYLE_KINEMATIC_COMMIT', {
        robot: robot.name,
  ...
  if (typeof window !== 'undefined') {
    ...
    window.MJCFViewer.__build = 'BUILD220_MUJOCO_STYLE_KINEMATIC_COMMIT';
