  ...
  async function renderList() {
    ...
    items.forEach((ent, index) => {
      ...
      row.addEventListener("pointerdown", (ev) => {
        // Keep panel interactions inside the panel even while the canvas is
        // listening for pointer gestures for joint dragging.
        ev.stopPropagation();
      });

      row.addEventListener("click", () => {
        // A component row now selects the actual kinematic link and then
        // isolates it. This makes the panel and 3-D selection describe the
        // same piece instead of behaving as unrelated controls.
        try { app.clearSelection?.(); app.interaction?.clearHover?.(); } catch (_) {}
        try { app.isolate.asset(ent.assetKey); } catch (_) {}
        try { app.selectAsset?.(ent.assetKey); } catch (_) {}
