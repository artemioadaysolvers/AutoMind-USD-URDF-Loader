...
export function createToolsDock(app, theme) {
  ...
  Object.assign(mechRow.style, { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', margin: '8px 0' });
  const btnJointAxes = mkToggleButton('Joint axes', false);
  const btnLoops = mkToggleButton('Loops', false);
  mechRow.appendChild(btnJointAxes);
  mechRow.appendChild(btnLoops);
  ...
  function updateSectionPlane() {
    ...
    if (!secEnabled || !app.robot) {
      sectionPlane = null;
      try { app.sectionPlane = null; app.getSectionPlane = () => null; } catch (_) {}
  ...
  btnJointAxes.addEventListener('click', () => {
    const on = !btnJointAxes.getActive();
    btnJointAxes.setActive(on);
    app.setMechanismToggles?.({ jointAxes: on });
  });
  btnLoops.addEventListener('click', () => {
    const on = !btnLoops.getActive();
    btnLoops.setActive(on);
    app.setMechanismToggles?.({ loops: on });
  });
