...
function viewIso(core, object = core.robot, duration = 260) {
  ...
  if (cam.isPerspectiveCamera) {
    ...
    dist = maxDim / Math.tan(Math.max(1e-6, fov / 2)) * 1.05;
  ...
  function step() {
    ...
    ctrl.target.lerpVectors(startTarget, center, k);
    ctrl.update();
...
function buildThumbnailer(core, assetToMeshes) {
  ...
  function hasLoadedImage(tex) {
    const img = tex?.image;
    return !!(img && ((typeof img.width === 'number' && img.width > 0) || img.complete === true));
  }
  async function waitForTextures(meshes, timeoutMs = 900) {
    const maps = [];
    for (const mesh of meshes || []) {
      for (const m of materialList(mesh?.material)) {
        if (m?.map && !hasLoadedImage(m.map)) maps.push(m.map);
      }
    }
  ...
  function cloneMaterialForPreview(src) {
    ...
    if (mat.map) {
      ...
      // Avoid invisible pure-white untextured parts without destroying their identity.
      const c = mat.color;
      if (c.r > 0.92 && c.g > 0.92 && c.b > 0.92) mat.color = new THREE.Color(0xdfe8ea);
    }
  ...
  async function thumbnail(assetKey) {
    ...
    for (const mesh of meshes) {
      ...
      try {
        ...
        const em = new THREE.LineBasicMaterial({ color: EDGE_COLOR, transparent: true, opacity: 0.40 });
