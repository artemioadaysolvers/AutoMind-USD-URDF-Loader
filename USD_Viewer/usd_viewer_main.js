...
function fadeMeshesVisible(core, meshes, duration = 520) {
  const states = [];
  ...
  for (const mesh of meshes || []) {
    ...
    for (const mat of materialList(mesh.material)) {
      ...
      seen.add(mat);
      const targetOpacity = Number.isFinite(mat.opacity) ? Math.max(0, mat.opacity) : 1;
      states.push({ mat, targetOpacity, transparent: !!mat.transparent, depthWrite: mat.depthWrite !== false });
  ...
  function step(now) {
    ...
    for (const st of states) {
      st.mat.opacity = st.targetOpacity * k;
    ...
    if (u < 1) requestAnimationFrame(step);
      ...
      for (const st of states) {
        ...
        st.mat.transparent = st.transparent || st.targetOpacity < 1;
        st.mat.depthWrite = st.depthWrite;
        st.mat.needsUpdate = true;
      }
    }
  }
...
function showAll(core) {
  ...
  const meshes = [];
  core.robot.traverse(o => { if (o.isMesh && o.geometry && !o.userData?.__isHoverOverlay) meshes.push(o); });
  fadeMeshesVisible(core, meshes, 560);
...
export function render(opts = {}) {
  ...
  const inter = attachInteraction({ scene: core.scene, camera: () => core.camera, renderer: core.renderer, controls: core.controls, robot, selectMode, getSectionPlane: () => app?.sectionPlane || null });
