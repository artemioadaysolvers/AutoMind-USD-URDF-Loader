...
function boxHasAnyVisibleSideByPlane(box, plane, eps = 1e-7) {
  ...
  // Three.js clipping discards the positive side of a Plane. Keep an object
  // pickable only when at least part of its world-space bounds remains on the
  // rendered side. This makes section-hidden components non-hoverable/non-clickable.
  return corners.some(c => plane.distanceToPoint(c) <= eps);
}
function meshVisibleBySection(mesh, plane) {
  if (!plane) return true;
  const box = new THREE.Box3().setFromObject(mesh);
  if (box.isEmpty()) return false;
  return boxHasAnyVisibleSideByPlane(box, plane);
}
...
export function attachInteraction({ scene, camera, renderer, controls, robot, selectMode = 'link', getSectionPlane = null, onSelectLink = null }) {
  ...
  function refreshSelectionMarker() {
    ...
    const visibleMeshes = (selectedMeshes || []).filter(m => m && m.visible !== false && meshVisibleBySection(m, plane));
  ...
  function pickInfoFromPointer(ev) {
    ...
    robotModel?.traverse?.(o => {
      if (o?.isMesh && o.geometry && o.visible !== false && !o.userData.__isHoverOverlay && meshVisibleBySection(o, plane)) pickables.push(o);
    });
