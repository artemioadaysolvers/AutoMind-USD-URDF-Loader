...
function meshHasKeptGeometryBySection(mesh, plane, eps = 1e-7) {
  ...
  // Sample actual vertices in world space so fully section-hidden parts cannot be
  // hovered, clicked, boxed, or dragged.
  ...
  for (let i = 0; i < count; i += step) {
    ...
    if (plane.distanceToPoint(__sectionVertexTmp) <= eps) return true;
  }
  ...
  return plane.distanceToPoint(__sectionVertexTmp) <= eps;
}
